...
\begin{tikzpicture}[scale=0.8]
    ...
    \node[anchor=west] at (10.3,5.4) {\footnotesize $\sim$90\% foundation};
    
    % Medieval
    \draw[fill=black!80] (0,4) rectangle (5,4.8);
    \draw[fill=black!20] (5,4) rectangle (10,4.8);
    \node[anchor=west] at (-3.5,4.4) {\footnotesize Medieval};
    \node[anchor=west] at (10.3,4.4) {\footnotesize $\sim$50\% foundation};
    
    % Pre-Bologna
    \draw[fill=black!80] (0,3) rectangle (2.5,3.8);
    \draw[fill=black!20] (2.5,3) rectangle (10,3.8);
    \node[anchor=west] at (-3.5,3.4) {\footnotesize Pre-Bologna};
    \node[anchor=west] at (10.3,3.4) {\footnotesize $\sim$25\% foundation};
    
    % Post-Bologna
    \draw[fill=black!80] (0,2) rectangle (0.6,2.8);
    \draw[fill=black!20] (0.6,2) rectangle (10,2.8);
    \node[anchor=west] at (-3.5,2.4) {\footnotesize Post-Bologna};
    \node[anchor=west] at (10.3,2.4) {\footnotesize $<$10\% foundation};
    
    % Micro-credential
    \draw[fill=black!20] (0,1) rectangle (10,1.8);
    \node[anchor=west] at (-3.5,1.4) {\footnotesize Micro-credential};
    \node[anchor=west] at (10.3,1.4) {\footnotesize 0\% foundation};
    
    % Legend - Stacked vertically on two lines
    % Line 1: Foundation
...
\end{tikzpicture}
\caption{The Foundation-Specialization Inversion (500 BCE--2024 CE). Proportions represent documented curriculum structures where available (medieval trivium/quadrivium requirements of 6--9 years before specialization \citep{history_hit_medieval}) and reasonable approximations for other periods. The argument rests not on numerical precision but on the unmistakable pattern: historical systems built comprehensive foundations before permitting specialization; modern systems invert this architecture. Ancient Greek \textit{paideia} was foundation-dominant; medieval universities mandated trivium and quadrivium for all students regardless of eventual specialty; post-Bologna degrees offer vestigial ``general education'' before immediate specialization; micro-credentials eliminate foundation entirely. The inversion correlates with declining extraction resistance: spherical foundations resist algorithmic capture, vectorized specializations invite it.}
...
\paragraph{A Necessary Clarification: Architecture, Not Quantity}

To forestall misreading: this analysis does not claim that total human knowledge was greater in pre-modern periods (obviously false), nor that broader educational access existed (equally false—medieval universities served a tiny fraction of society). The claim is architectural: among those who received formal education, foundational breadth preceded specialization, creating cognitive substrate enabling productive \textit{inter}-disciplinary engagement.

The evidence is unambiguous. All medieval university students were required to study the seven liberal arts—trivium (grammar, logic, rhetoric) followed by quadrivium (arithmetic, geometry, music, astronomy)—before any specialization in theology, law, or medicine \citep{wikipedia_medieval_university, history_hit_medieval}. As university statutes declared: no student could advance to higher faculties without demonstrated mastery of this comprehensive foundation. The process required six to nine years \citep{history_hit_medieval}. A theologian and a physician, having both completed trivium and quadrivium, could dispute causation across their domains—generating insight through friction between different specialized lenses applied to shared foundational concepts.

Contemporary specialists, lacking common cognitive substrate, engage primarily in \textit{intra}-disciplinary disputes—the same lens arguing about methodological minutiae, generating heat without insight. The shift from inter-disciplinary to intra-disciplinary conflict marks the thermodynamic transition from sphere to vector architecture.

Guild apprenticeships achieved extraction resistance through a different mechanism. A carpentry apprentice "learned to \textit{do} geometry without learning \textit{about} geometry" \citep{medieval_buildings_carpenters}—practical methods passed through demonstration rather than formal instruction. This represents Collins's somatic tacit knowledge \citep{collins2010}: unextractable precisely because it was never formalized. The guild master possessed what we would now recognize as high R-value expertise, but grounded in embodied practice rather than theoretical breadth.

Historical education thus offered two paths to extraction resistance: (1) universities building broad theoretical foundations enabling cross-domain synthesis; (2) guilds cultivating deep embodied practice resistant to formalization. Modern micro-credentials possess \textit{neither}—no broad foundation AND no deep embodiment. Pure vectorization with no thermodynamic anchor.

\paragraph{Contemporary Proof: The Veterinarian as Surviving Sphere}

Lest this appear merely historical nostalgia, consider a contemporary comparison operating under identical time constraints: veterinary versus human medical education.

Both require approximately the same training duration. Yet veterinary curricula mandate comparative medicine across five to seven major species groups—canine, feline, equine, bovine, porcine, and often exotics—while human medical training focuses exclusively on \textit{Homo sapiens} \citep{ross_vet_school, pmc_veterinary_value}. This structural difference produces measurably different cognitive architectures.

Human physicians rely heavily on patient-reported symptoms ("I feel a sharp pain here"). Veterinarians, treating patients who cannot speak, must rely almost entirely on clinical signs—observable, objective physical evidence—and pattern recognition across vastly different biological systems \citep{pmc_veterinary_value, jvme_clinical_reasoning}. Experienced veterinarians develop what researchers call a "scanning" approach: simultaneously assembling data and hypotheses rather than following linear checklists \citep{jvme_clinical_reasoning}. This is \textit{metis}—adaptive cunning across contexts—structurally absent from specialist human medical training.

The veterinary profession's self-description captures the architectural difference: "Real Doctors Treat More Than One Species" \citep{pmc_veterinary_value}. This reflects genuine cognitive difference: veterinarians are generalists who must adapt medical principles to novel biological contexts (extrapolating drug dosages from cat to tiger), whereas human specialists deepen knowledge within a known, fixed biological system.

Same training investment. Different architecture. Different extraction resistance. The human specialist in "the right small finger" possesses deep episteme in one context but minimal adaptive capacity. The veterinarian develops pattern recognition across body plans that the specialist cannot access. When AI systems trained on human medical literature attempt veterinary diagnosis, they fail—not from lack of data but from architectural mismatch. The sphere resists what the vector invites.

This contemporary example validates the historical pattern: foundational breadth enables cross-domain synthesis that resists algorithmic extraction. The question is not whether such architecture remains possible but whether our educational systems still produce it.
